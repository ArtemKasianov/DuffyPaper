...
def main():
    ...
    parser.add_argument('--outputDistrIntermVals', type=str, default=None,
                        help='Optional path to distr interm values file ')
    ...
    with open(args.outputDistrIntermVals, "w") as f:
        for curr_interm_freq in interm_freqs:
            f.write(f"{curr_interm_freq}\n")
    ...
    plt.title(f'Distribution of final allele A frequencies\n'
              f'(N={args.N}, p0={args.p0}, generations={args.generations2}, iterations={args.iterations})')
    plt.grid(True)
