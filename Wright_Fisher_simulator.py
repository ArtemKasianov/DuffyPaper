...
def wright_fisher_simulation_sp(N, p0, generations1, generations2, sample_size_val, seed=None):
    """
    Perform two stage Wright-Fisher sampling simulation for single pulse admixture.
    ...
    # Calculate frequency of 'A' in the sample
    freq_B = sample.count('A') / sample_size_val
...
def main():
    ...
    parser.add_argument('--single_pulse', type=int, default=None,
                        help='For single pulse use 1, elsewhere continuous gene flow model')
    ...
    for i in range(args.iterations):
        ...
        (interm_p,final_p) = (0.0,0.0)
        if args.single_pulse == 1:
            (interm_p,final_p) = wright_fisher_simulation_sp(args.N, args.p0, args.generations1,args.generations2, int(args.sample_size), sim_seed)
        else:
